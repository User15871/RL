# -*- coding: utf-8 -*-
"""
Created on Mon Jul 26 17:41:39 2021
@author: andy
"""
...
def filter_batch(batch, percentile):
    ...
    train_act = []
    elite_batch = []
    for sample, discounted_reward in zip(batch, disc_rewards):
        if discounted_reward > reward_bound or discounted_reward == max(disc_rewards):
            train_obs.extend(map(lambda step: step.observation, sample.steps))
            train_act.extend(map(lambda step: step.action, sample.steps))
            elite_batch.append(sample)
...
if __name__ == "__main__":
    ...
    for iter_no, batch in enumerate(iterate_batches(env, net, BATCH_SIZE)):
        ...
        if reward_mean > 0.8 or iter_no == 10000:
            env.close()
            print("Solved!")
            break
    writer.close()
